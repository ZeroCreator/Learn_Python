...
# Вам дан список numbers, в нем есть и отрицательные числа и нули и положительные значения. Гарантируется, что список
# целиком состоит только из чисел.
...
print(len(list(filter(lambda x: x < 0, numbers))), len(list(filter(lambda x: x == 0, numbers))), len(list(filter(lambda x: x > 0, numbers))))

#
print(*[len(list(filter(fnc, numbers))) for fnc in [lambda x: x < 0, lambda x: x == 0, lambda x: x > 0]])

#
func = [lambda x: x<0, lambda x: x==0, lambda x: x>0]
for i in func:
    print(len(list(filter(i, numbers))), end = ' ')

#
print(*[eval(f'len(list(filter(lambda x: x {_} 0, numbers)))') for _ in ['<', '==', '>']])
...
d = list(filter(lambda x: len(x) == 4 or x[0] == 'S', days))
for i in sorted(d):
    print(i)
